...
def create_bin_dataset(D, N, no=0):
    ...
    @return: a tuple of (x, y, w, b), where
    w: with shape of (D)
    b: bias, (1,)   wx+b=0 is the separated plane
    x: with shape of (N,D).
    y: target, (N,)   element value is in {1,-1}
    '''
...
def create_dataset_linear(D, N, C):
    ...
    a linearly distribution. All points of a class oscillate around some line.
    ...
    we can map these datas into a space with a higher dimension(e.g. 3-dim),
    ...
    this method generates points in this distribution form, so C-1 seperated planes
    are parallel, with the same normal vector and different biases.
    ...
    @returns:
        ...
        w: with a shape of (D)     the C-1 parallel seperated planes normal vector
        b: with a shape of (C-1),   biases of the C-1 parallel superplanes
...
def create_dataset_cylindrical(D, N, C):
    ...
    feature space is (R^D), the subspace R^2 of the first 2 dimension is partitioned into 
    C parts, and the complementary subpace has no relation with classification.

    example: take the first 2 dimension, project all points into this subspace, and then 
        the points are shown as following:
    ...
    @parameters:
        D: number of features   (x's dimension)
        N: dataset size
        C: number of classes
    @return:
        x: (N, D) features of all points
        y: (N,)     class indices (range: [0, C-1])
        thetas: (C,) angle boundaries in first 2 dim subspace of C classes 
            thetas[0]~thetas[1]     -> 1-st class
            thetas[1]~thetas[2]     -> 2-nd class
            ...
            thetas[C-2]~thetas[C-1] -> (C-1)-th class
            thetas[C-1]~thetas[0]   -> C-th class
    '''
    assert D > 1, 'Number of features must be larger than 1, but got: %d' % D


    mean_theta = 2 * torch.pi / C
    delta_theta = mean_theta * 0.15      # angle amplitude, max delta of angle, when disturbed
    delta_thetas = torch.rand(C) * delta_theta  # randomly generates the angle deltas
    delta_thetas[torch.rand(C) > 0.5] *= -1 # with prob 1/2, make angle deltas using their inverse numbers

    # store angle boundaries of all classes
    thetas = torch.arange(C+1) * mean_theta         # (C+1), balance-partition 2 pi into C parts
    thetas[:-1] += delta_thetas         # disturb boundaries of all classes
    thetas[thetas < 0] += torch.pi * 2  # calibrate negative angles
    ...
    thetas[-1] = thetas[0]              # the last and the first is equal, makes a circle.

    mn = MultivariateNormal(torch.zeros(D), torch.eye(D))
    x = mn.sample(torch.Size([N]))      # points, (N, D)
    y = torch.zeros(N, dtype=torch.int16)
    norm = torch.norm(x[:,:2], dim=1)   # sqrt(x1*x1+x2*x2), (N,)
